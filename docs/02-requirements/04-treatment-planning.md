...
The plan displays clinic fee, discount, and patient amount. No insurance fields, estimates, payer balances, deductibles, or claim data exist. Money follows ADR-005's canonical rial/toman rules; the plan-specific addition is that a presented plan version snapshots each amount in canonical rials, so a later fee change never alters an already-presented plan.
