...
56. Posted entries are immutable.

57. Corrections use a reversal and a new correct entry.

58. The canonical monetary unit is the Iranian rial (IRR), stored as a signed 64-bit integer number of rials. JavaScript floating-point values are not authoritative for money.

59. Every entry records patient, business date, timestamp, author, type, canonical rial amount, source, and reason where applicable.

60. A payment may be allocated to charges, but the unallocated amount remains explicit.

61. A refund references the original payment when possible.

62. Balance is derived from posted entries; it is not independently editable.

63. Clinical completion may create a draft charge or posted charge according to office configuration.
...
## Rial and toman presentation
  ...
  - One toman equals exactly ten rials. Toman input is converted to canonical rials at the application boundary.

  - Every editable money control, total, report, receipt, statement, and export MUST identify its unit. Bare unlabeled financial values are prohibited.

  - If a rial amount is not divisible by ten, a toman display must preserve the exact value (for example as a decimal toman) or fall back to rial; it must not silently round.
  ...
  - Digit localization and grouping are display concerns; arithmetic always uses canonical numeric values.
...
## Receipts and statements
  ...
  - Persian, English, and bilingual templates
