...
**3 of 5 proven now (2 in S2, 1 post-S8). Status stays Proposed** — accepting the ADR is a governance call, not an implementation detail; see the human-review note in the S2 write-up. The remaining 2 (API-leg mapping, money DB↔API round trip) each need a real endpoint/table this release doesn't have a product reason to build yet — not stalled, just genuinely blocked on later feature work per `00-build-sequencing.md`.
