...
The UI design system (`docs/03-ux/05-ui-design-system.md`) implements this ADR through fixed
public bootstrap values: `locale=fa-IR`, `dir=rtl`, and `calendarDisplay=JALALI`. The shell
validates them before rendering and exposes no language/calendar switch. Adding another locale
requires a replacement ADR, translation resources, LTR implementation, and expanded verification.
...
- Reintroducing English (or any second locale/LTR) requires a replacement ADR and budget for translation, LTR visual QA, and localized print templates.
