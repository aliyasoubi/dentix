...
**Scope note (`07-plans/00-build-sequencing.md`):** the Logging rules below apply from Release 1 — the never-log list is a day-one security requirement. Dashboards, tracing, SLO alerting, and the domain-health signals are the pre-pilot target, built before Release 7, not in the first foundation.
