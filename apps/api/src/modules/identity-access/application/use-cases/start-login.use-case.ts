 ...
 * lives in OidcAuthorizationRequest.create — this use case doesn't repeat it.
 */
@Injectable()
export class StartLoginUseCase {
  ...
  async execute(params: { readonly returnPath: string }): Promise<{ readonly authorizationUrl: URL }> {
    const now = new Date();
