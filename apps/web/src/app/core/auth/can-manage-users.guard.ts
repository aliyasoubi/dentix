...
/**
 * Route-level convenience only, same as the canManageUsers field it reads —
 * AddOfficeUserUseCase re-checks independently server-side (via
 * AuthorizationPort's fresh "user.manage" resolution), which is the actual
 * gate. This exists so a user who navigates or deep-links to the add-user
 * page without that permission lands somewhere sensible instead of a form
 * that will only ever come back FORBIDDEN.
 *
 ...
 */
export const canManageUsersGuard: CanActivateFn = async () => {
